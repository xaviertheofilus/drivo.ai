...
class DrivoAITester:
    ...
    def wait_for_backend(self, max_wait_seconds: int = 60) -> bool:
        url = f"{self.base_url}/"
        start = time.time()
    ...
    def test_personas_edit_template(self):
        """Test editing a template persona (creates user override)"""
        self.log("=== PERSONAS: Edit Template ===", "SECTION")
        _, templates = self.run_test(
            "Get templates for edit",
            "GET",
            "personas/templates",
            200
        )
        ...
        if not template_id:
            return False
        new_name = f"{templates[0].get('name', 'Template')} (Edited)"
        success, _ = self.run_test(
            "Edit template persona (PUT /personas/{id})",
            "PUT",
            f"personas/{template_id}",
            200,
            data={
                "name": new_name,
                "voice_tone": "calm",
        ...
        if not success:
            return False
        _, templates2 = self.run_test(
            "Get templates after edit",
            "GET",
            "personas/templates",
            200
        )
        if not templates2 or not isinstance(templates2, list):
            return False
        edited = next((t for t in templates2 if t.get('id') == template_id), None)
        return edited and edited.get('name') == new_name and edited.get('voice_tone') == "calm"
    ...
    def test_personas_limit(self):
        ...
        _, existing = self.run_test(
            "Get existing personas for limit check",
            "GET",
            "personas",
            200,
        )
        ...
        if isinstance(existing, list):
            existing_count = sum(1 for p in existing if p.get("type") in ("custom", "auto"))
    ...
    def test_sessions_language_auto(self, session_id: str):
        """Test auto language detection EN/ID"""
        success, response = self.run_test(
            "Turn with language=auto (Indonesian text)",
            "POST",
            f"sessions/{session_id}/turn",
            200,
            data={
                "text": "Aku lagi nyetir, tolong jawab singkat ya.",
                "language": "auto",
                "latency_ms": 400,
                "silence_seconds": 3,
                "speech_energy": 0.6
        ...
        return success and response.get("language") == "id" and isinstance(response.get("ai_reply"), str)
    ...
    def run_all_tests(self):
        ...
        if not self.wait_for_backend():
            self.log("Backend not reachable - stopping tests", "FAIL")
            return False
        ...
        self.test_personas_edit_template()
        ...
        if session_id:
            ...
            self.test_sessions_language_auto(session_id)
