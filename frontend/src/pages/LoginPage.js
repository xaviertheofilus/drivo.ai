...
export default function LoginPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Welcome back to DrivoAI');
      navigate('/dashboard');
    } catch (err) {
    ...
    <div className="min-h-screen grid lg:grid-cols-12" style={{ background: 'var(--bg)' }}>
      ...
      <div className="hidden lg:flex lg:col-span-5 flex-col justify-between p-10 hero-bg relative" style={{ borderRight: '1px solid var(--border)' }}>
        ...
        <div>
          <p className="font-display text-3xl leading-tight max-w-md">
            “The road is long.<br />You don't drive it alone.”
          </p>
          <p className="mt-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
            A voice companion that adapts to you — in EN or ID.
          </p>
        </div>
      ...
      <div className="lg:col-span-7 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md">
          ...
          <p className="mt-2 text-sm" style={{ color: 'var(--text-secondary)' }}>Sign in to continue your drive.</p>

          <form onSubmit={onSubmit} className="mt-8 space-y-5">
            <div>
              <Label htmlFor="email" className="text-xs uppercase tracking-widest font-mono" style={{ color: 'var(--text-secondary)' }}>Email</Label>
              <Input
                id="email"
            ...
            <div>
              <Label htmlFor="password" className="text-xs uppercase tracking-widest font-mono" style={{ color: 'var(--text-secondary)' }}>Password</Label>
