...
export default function PersonaBuilderFilePage() {
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      const draft = {
        ...
        extracted_text_preview: data?.text_preview || null,
        avatar_file_id: null,
      };
      localStorage.setItem('drivoai_persona_draft', JSON.stringify(draft));
      navigate('/personas/new/preview');
    } catch (e) {
      setError(e?.response?.data?.detail || t('personaBuilder.couldNotGenerate'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="pb-24 lg:pb-0">
      ...
      <p className="text-xs uppercase tracking-[0.25em] font-mono mt-4" style={{ color: 'var(--primary)' }}>{t('personaBuilder.step1Upload')}</p>
      <h1 className="font-display text-3xl sm:text-4xl mt-2">{t('personaBuilder.dropTitle')}</h1>
      <p className="text-sm mt-2" style={{ color: 'var(--text-secondary)' }}>{t('personaBuilder.dropHint')}</p>

      <div className="mt-8">
        <Label className="text-xs uppercase tracking-widest font-mono" style={{ color: 'var(--text-secondary)' }}>{t('personaBuilder.sourceDocument')}</Label>
        <div
        ...
        >
          {!file && (
            <label className="flex flex-col items-center justify-center gap-3 py-16 cursor-pointer">
              ...
              <p className="text-sm" style={{ color: 'var(--text-primary)' }}>{t('personaBuilder.dropHere')}</p>
              <p className="text-[11px] font-mono" style={{ color: 'var(--text-muted)' }}>{t('personaBuilder.formatsHint')}</p>
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-3">
        ...
        >
          <Wand2 className="h-4 w-4 mr-2" />{loading ? t('personaBuilder.readingGenerating') : t('personaBuilder.generatePersona')}
        </Button>
        <Button asChild variant="ghost" className="h-12 px-6"><Link to="/personas/new/text">{t('personaBuilder.switchToText')}</Link></Button>
      </div>
    </div>
  );
}
