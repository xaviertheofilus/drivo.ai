...
export default function DashboardPage() {
  ...
  const startSession = async (withPersona) => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.detail || t('dashboard.couldNotStartSession'));
    } finally {
      setStarting(false);
    }
  };
  ...
  return (
    <div className="pb-24 lg:pb-0">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          <p className="text-xs uppercase tracking-[0.25em] font-mono" style={{ color: 'var(--primary)' }}>{t('dashboard.title')}</p>
      ...
      <div className="mt-8 grid lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 card-surface p-6 sm:p-8">
          <p className="text-xs uppercase tracking-widest font-mono" style={{ color: 'var(--text-secondary)' }}>{t('dashboard.activePersona')}</p>
          {active ? (
            ...
            <div className="mt-4">
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{t('dashboard.noPersonaSet')}</p>
            </div>
          )}
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            ...
            >
              {starting ? t('dashboard.starting') : (
                <span className="flex items-center gap-2">
                  {t('dashboard.startSession')} <ArrowRight className="h-4 w-4" />
                </span>
              )}
            </Button>
            <Button asChild variant="outline" className="h-14 px-6 text-base" style={{ background: 'var(--surface-alt)', borderColor: 'var(--border)' }} data-testid="dashboard-change-persona-link">
              <Link to="/personas">{active ? t('dashboard.changePersona') : t('dashboard.choosePersona')}</Link>
            </Button>
          </div>
        </div>
        ...
        <div className="card-surface p-6">
          <p className="text-xs uppercase tracking-widest font-mono" style={{ color: 'var(--accent)' }}>{t('dashboard.talkFirst')}</p>
          <h3 className="font-display text-2xl mt-2">{t('dashboard.noPersonaTitle')}</h3>
          <p className="text-sm mt-2" style={{ color: 'var(--text-secondary)' }}>
            {t('dashboard.noPersonaDesc')}
          </p>
          <Button
      ...
      <div className="mt-12">
        <div className="flex items-end justify-between">
          <div>
            <p className="text-xs uppercase tracking-widest font-mono" style={{ color: 'var(--text-secondary)' }}>{t('dashboard.recentDrives')}</p>
            <h3 className="font-display text-2xl mt-1">{t('dashboard.lastFew')}</h3>
          </div>
          <Link to="/history" className="text-sm" style={{ color: 'var(--primary)' }} data-testid="dashboard-view-history-link">{t('dashboard.viewAll')}</Link>
        </div>
        <div className="mt-4 card-surface overflow-hidden">
          {recent.length === 0 && (
            <div className="p-10 text-center">
              <Mic className="mx-auto h-8 w-8 mb-3" style={{ color: 'var(--text-muted)' }} />
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{t('dashboard.noDrives')}</p>
            </div>
          )}
          {recent.length > 0 && (
