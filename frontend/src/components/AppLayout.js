...
import { Home, Users2, Clock, Settings, LogOut, Mic } from 'lucide-react';
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar';
import { fileUrl } from '@/lib/api';

const NAV = [
  { to: '/dashboard', label: 'Home', icon: Home, testId: 'nav-home' },
  { to: '/personas', label: 'Personas', icon: Users2, testId: 'nav-personas' },
...
export function AppLayout({ children }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--bg)' }}>
      {/* Sidebar (desktop) */}
      <aside className="hidden md:flex flex-col w-60 shrink-0 border-r" style={{ borderColor: 'var(--border)', background: 'rgba(18,18,26,0.6)' }}>
        ...
        <div className="p-3 border-t" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-3 px-2 py-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm truncate" style={{ color: 'var(--text-primary)' }}>{user?.full_name || 'Driver'}</p>
              <p className="text-[11px] truncate" style={{ color: 'var(--text-muted)' }}>{user?.email}</p>
            </div>
            <button data-testid="nav-logout" onClick={async () => { await logout(); navigate('/login'); }} className="p-2 rounded-md hover:bg-white/10" aria-label="Logout">
      ...
      <main className="flex-1 min-w-0">
        {/* Mobile top bar */}
        <div className="md:hidden flex items-center justify-between px-4 h-14 border-b" style={{ borderColor: 'var(--border)' }}>
        ...
        {/* Mobile bottom nav */}
        <div className="md:hidden fixed bottom-0 left-0 right-0 border-t backdrop-blur z-40" style={{ borderColor: 'var(--border)', background: 'rgba(10,10,15,0.85)' }}>
