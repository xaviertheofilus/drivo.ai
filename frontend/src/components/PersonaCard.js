...
export const PersonaCard = ({
  ...
  return (
    ...
    >
      ...
      {sample.ai && (
        <div className="mt-4 rounded-md p-3" style={{ background: 'var(--surface-alt)', borderLeft: '2px solid var(--primary)' }}>
          <p className="italic text-sm" style={{ color: 'var(--text-primary)' }}>“{sample.ai}”</p>
          {sample.ai_followup && (
            <p className="italic text-xs mt-1" style={{ color: 'var(--text-muted)' }}>then: “{sample.ai_followup}”</p>
          )}
        </div>
      )}
